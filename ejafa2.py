...
class EjafaProtocol:
    ...
    def derive_key(self, shared_secret: bytes, peer_public_key: bytes) -> Tuple[bytes, bytes]:
        """Derive a session key and key confirmation value from the shared secret using BLAKE2b"""
        key = bytearray(SESSION_KEY_SIZE)
        key_confirmation = bytearray(KEY_CONFIRMATION_SIZE)
        ...
        # Derive session key
        buffer = bytearray(40)
        buffer[0:8] = struct.pack("<Q", 1)  # subkey_id = 1
        buffer[8:16] = context
        ...
        # Derive key confirmation value
        # Sort public keys to ensure consistent ordering
        sorted_keys = sorted([self.public_key, peer_public_key])
        
        buffer = bytearray(72)  # Increased buffer size to hold both public keys
        buffer[0:8] = struct.pack("<Q", 2)  # subkey_id = 2
        buffer[8:16] = context
        buffer[16:24] = struct.pack("<Q", KEY_CONFIRMATION_SIZE)
        buffer[24:56] = sorted_keys[0]  # First public key
        buffer[56:88] = sorted_keys[1]  # Second public key

        h = hashlib.blake2b(key=shared_secret, digest_size=32)
        for _ in range(ROUNDS):
            h.update(buffer)
        key_confirmation = h.digest()[:KEY_CONFIRMATION_SIZE]

        print(f"{self.name}: deriveKey - Keys derived from shared secret")
        print(f"{self.name}: Derived Key (hex): {derived_key.hex()}")
        print(f"{self.name}: Key Confirmation (hex): {key_confirmation.hex()}")
        
        self.log_key(derived_key, "Derived Key")
        self.log_key(key_confirmation, "Key Confirmation")

        return derived_key, key_confirmation

    def verify_key_confirmation(self, key_confirmation: bytes, peer_public_key: bytes, shared_secret: bytes) -> bool:
        """Verify the key confirmation value from the peer"""
        expected_confirmation = bytearray(KEY_CONFIRMATION_SIZE)
        ...
        for _ in range(ROUNDS):
            h.update(buffer)
        expected_confirmation = h.digest()[:KEY_CONFIRMATION_SIZE]

        return key_confirmation == expected_confirmation
...
def main():
    ...
    alice_key, alice_key_confirmation = alice.derive_key(alice_shared_secret, bob_public_key)
    bob_key, bob_key_confirmation = bob.derive_key(bob_shared_secret, alice_public_key)

    # Verify key confirmation
    if not alice.verify_key_confirmation(bob_key_confirmation, bob_public_key, alice_shared_secret):
        print("ERROR: Bob's key confirmation failed!")
        return

    if not bob.verify_key_confirmation(alice_key_confirmation, alice_public_key, bob_shared_secret):
        print("ERROR: Alice's key confirmation failed!")
        return
